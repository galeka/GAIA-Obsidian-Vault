...
5. **Update the registry.** If the change affects the agent's description, triggers, or capabilities, update the corresponding row in `.claude/references/agents-registry.md`. Custom agent rows live between the `<!-- GAIA:CUSTOM_AGENTS_START -->` and `<!-- GAIA:CUSTOM_AGENTS_END -->` markers — edit only within that block.
