...
5. **Update the registry**: add a new row to `.claude/references/agents-registry.md` — insert it between the `<!-- GAIA:CUSTOM_AGENTS_START -->` and `<!-- GAIA:CUSTOM_AGENTS_END -->` markers in the Registry table (after the postman row)
6. **Update the directory**: add a new section under "Custom Agents" in `.claude/references/agents.md` — insert it between the `<!-- GAIA:CUSTOM_AGENTS_START -->` and `<!-- GAIA:CUSTOM_AGENTS_END -->` markers in that file
